...
t_welch_small <- t.test(xs, ys)
t_welch_large <- t.test(xl, yl)

t_pooled_small <- t.test(xs, ys, var.equal = TRUE)
t_pooled_large <- t.test(xl, yl, var.equal = TRUE)

t_welch_small$p.value
t_welch_large$p.value
t_pooled_small$p.value
t_pooled_large$p.value
...
df_value %>%
  ...
  geom_segment(data =df_mu%>%
                 filter(group %in% c("a1", "a2")), 
               aes(x = group, xend = group, y=mu - sd, 
                   yend = mu + sd)) +
...
## 10.5.3 Bonus Exercise:Welch's t-test  
